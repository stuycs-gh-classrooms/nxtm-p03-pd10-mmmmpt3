...
void windTunnelSetup() { //  line( 400, 600, 400, 400);
...
void keyPressed() {
  ...
  if (toggles[WINDTUNNEL]) {
    windTunnelSetup();
    if (key == 'o') {
      test = new Orb(400, 390, 100, int(random(MIN_MASS, MAX_MASS)));
      D_COEF = .5;
    } else if (key == 'd') {
      test= new teardrop(400, 390, 100, int(random(MIN_MASS, MAX_MASS)));
      D_COEF = .045;
    } else if (key == 'r') {
      test= new rectangle(400, 390, 100, int(random(MIN_MASS, MAX_MASS)));
      D_COEF = .295;
    } else if (key == 't') {
      test= new triangle(400, 390, 100, int(random(MIN_MASS, MAX_MASS)));
      D_COEF = 1.14;
    }
  }
...
void runWindTunnel() {
  ...
  if (test != null) {
    ...
    if (toggles[MOVING]) {
      ...
      text(test.velocity.toString(), 500, 700 );
    }
  }
